...
pages = [
  {
    ...
    parser: ( result, config, newResponse ) ->
      if ( ! result )
        newResponse += '! Nenašiel som denné menu !\n'
      today = moment().day() - 1
      rowsPerDay = 4
      ...
      while i < ( rowStart + rowsPerDay )
        ...
        data = result.table[i]
        test = data.toLowerCase().indexOf('polievka')
        # console.log('test', test)

        if ( test > -1 )
          data = data.slice( test + 8 )
          data = data.replace('l', 'l ')

        test = data.toLowerCase().indexOf('a)')
        if ( test > -1 )
          data = data.slice( test + 2 )

        test = data.toLowerCase().indexOf('b)')
        if ( test > -1 )
          data = data.slice( test + 2 )

        test = data.toLowerCase().indexOf('bezmäsité menu')
        if ( test > -1 )
          data = data.slice( test + 14 )
